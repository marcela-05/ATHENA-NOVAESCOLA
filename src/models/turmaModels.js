...
turmas.prototype.postTurma = function(callback, idProfessor, idDisciplina, nomeTurma, serieTurma) {
    ...
    var sql = 'INSERT INTO turma (nome, serie, id_professor) VALUES ( "' + 
    nomeTurma + '",' + serieTurma + ',' + idProfessor + ');';
    var erro = ''
    database.appDB.all(sql, [], (err, rows) => {
        if (err) {
            console.error(err.message);
    ...
    if(erro.length < 1){
        sql = 'INSERT INTO turma_disciplina VALUES ((SELECT id_turma FROM turma WHERE nome = "' + nomeTurma +
        '" AND id_professor = ' + idProfessor + '), ' + idDisciplina + ');';
        database.appDB.all(sql, [], (err, rows) => {
            if (err) {
                console.error(err.message);
                callback(err.message)
